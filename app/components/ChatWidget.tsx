...
export default function ChatWidget({
  ...
  return (
    <div className={isEmbed ? "w-full h-screen font-sans" : "fixed bottom-6 right-6 z-9999 font-sans"}>
      
      {/* 3. Floating Button: Shudhu tokhon dekhabo jokhon embed hobe na */}
      {!isEmbed && (
        <motion.button
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            // 4. Animation logic: Embed hole initial animation-er proyojon nai
            initial={isEmbed ? {} : { opacity: 0, y: 20, scale: 0.95 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={isEmbed ? {} : { opacity: 0, y: 20, scale: 0.95 }}
            className={
              isEmbed
                ? "w-full h-full bg-white flex flex-col overflow-hidden" // Full Screen for Iframe
                : "absolute bottom-20 right-0 w-95 sm:w-105 h-150 bg-white border border-gray-100 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] flex flex-col overflow-hidden"
            }
          >
            {/* Header */}
            <div className="p-5 bg-linear-to-r from-blue-600 to-indigo-700 text-white shadow-md">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-white/70 text-[11px] uppercase font-medium tracking-widest">
                        Active Now
                      </span>
                    </div>
                  </div>
